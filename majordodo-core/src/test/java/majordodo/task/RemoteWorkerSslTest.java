 ...
 */
public class RemoteWorkerSslTest extends SimpleBrokerSuite {

    NettyChannelAcceptor server;

    @Override
    protected BrokerLocator createBrokerLocator() {
        NettyBrokerLocator nb = new NettyBrokerLocator(server.getHost(), server.getPort(), true);
        nb.setSslUnsecure(true);
        return nb;
    }
