 ...
 */
public class ReplicatedEnvSSlTest extends SimpleBrokerSuite {
    ...
    protected BrokerLocator createBrokerLocator() throws Exception {
        ZKBrokerLocator zk = new ZKBrokerLocator(zkEnv.getAddress(), zkEnv.getTimeout(), zkEnv.getPath());
        zk.setSslUnsecure(true);
        return zk;
    }

    @Override
    protected StatusChangesLog createStatusChangesLog() throws Exception {
        return new ReplicatedCommitLog(zkEnv.getAddress(), zkEnv.getTimeout(), zkEnv.getPath(), workDir,
            BrokerHostData.formatHostdata(new BrokerHostData(host, port, "", true, null)), false);
    }
