...
/**
 * Copia di java.io.BufferedInputStream non sincronizzata e final
 *
 * @author enrico.olivelli
 */
public final class SimpleBufferedInputStream extends FilterInputStream {

    private static int defaultBufferSize = SystemPropertyUtils.getIntSystemProperty("magnews.io.simplebufferstream.buffersize", 64 * 1024);
    /**
    ...
    protected volatile byte buf[]; // MUST be volatile for AtomicReferenceFieldUpdater
    /**
     * Atomic updater to provide compareAndSet for buf. This is necessary because closes can be asynchronous. We use
     * nullness of buf[] as primary indicator that this stream is closed. (The "in" field is also nulled out on close.)
     */
    private static final AtomicReferenceFieldUpdater<SimpleBufferedInputStream, byte[]> bufUpdater
        = AtomicReferenceFieldUpdater.newUpdater(SimpleBufferedInputStream.class, byte[].class, "buf");
    /**
     ...
     * @exception IOException if this input stream has been closed by invoking its {@link #close()} method, or an I/O
     * error occurs.
     ...
     * following conditions becomes true: <ul>
     *
     ...
     *
     * @exception IOException if the stream does not support seek, or if this input stream has been closed by invoking
     ...
     * <code>count&nbsp;- pos</code>) and the result of calling the {@link java.io.FilterInputStream#in in}.available().
     *
